	...
	STRBUF *files;
{
	...
	fputs_nl(gen_page_begin(title_file_index, TOPDIR, 0), FILES);
	...
	while ((_ = strbuf_fgets(input, FIND, STRBUF_NOCRLF)) != NULL) {
		...
		if (count_stack(push) || count_stack(pop)) {
			...
			while (count_stack(push)) {
				...
				strbuf_puts(sb, path);
				strbuf_putc(sb, '/');
				fputs_nl(gen_page_begin(strbuf_value(sb), SUBDIR, 0), STDOUT);
...
makeincludeindex(void)
{
	...
	for (inc = first_inc(); inc; inc = next_inc()) {
		...
		if (inc->count > 1) {
			...
			fputs_nl(gen_page_begin(last, SUBDIR, 0), INCLUDE);
