 ...
 *
 *	i)	ctags_x	raw output from global(1)
...
void
put_converting(const char *ctags_x, int absolute, int cxref, int fileid)
{
	SPLIT ptable;
	char buf[MAXPATHLEN+1];
	const char *fid, *path, *p = ctags_x;
	...
	if (cxref) {
		/*
		 * Put file id to the head of the output.
